-- Fix get_platform_tenant_detail runtime errors (jsonb_agg ORDER BY, optional CRM tables)
...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    'recentScans', (
      SELECT COALESCE(jsonb_agg(to_jsonb(s) ORDER BY s."scannedAt" DESC), '[]'::jsonb)
      FROM (
        SELECT
          sl.id,
    ...
    'recentClients', (
      SELECT COALESCE(jsonb_agg(to_jsonb(c) ORDER BY c."enrolledAt" DESC), '[]'::jsonb)
      FROM (
        SELECT
          cl.id,
    ...
    'recentRewards', (
      SELECT COALESCE(jsonb_agg(to_jsonb(r) ORDER BY r."createdAt" DESC), '[]'::jsonb)
      FROM (
        SELECT
          rw.id,
          c.full_name AS "clientName",
          rw.reward_description AS "rewardDescription",
          rw.redeemed_at AS "redeemedAt",
...
EXCEPTION
  WHEN undefined_column THEN
    RAISE EXCEPTION 'Migration requise: appliquez les migrations 024–028 (colonnes manquantes)';
END;
$$;
