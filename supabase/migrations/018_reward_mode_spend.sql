-- Spend-based loyalty rewards (alternative to stamp count)

ALTER TABLE shop_settings
  ADD COLUMN IF NOT EXISTS reward_mode TEXT NOT NULL DEFAULT 'stamps'
    CHECK (reward_mode IN ('stamps', 'spend')),
...
UPDATE shop_settings SET reward_mode = 'stamps' WHERE reward_mode IS NULL;

-- Client card RPC: expose spend progress
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'rewardMode', COALESCE(v_settings.reward_mode, 'stamps'),
