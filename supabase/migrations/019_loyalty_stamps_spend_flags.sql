-- Independent stamp / spend toggles (owner can enable one or both)
...
-- Migrate legacy reward_mode if present
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'shop_settings' AND column_name = 'reward_mode'
  ) THEN
    UPDATE shop_settings SET
      stamps_enabled = reward_mode IN ('stamps', 'both'),
      spend_enabled = reward_mode IN ('spend', 'both');
  END IF;
END $$;

ALTER TABLE shop_settings DROP CONSTRAINT IF EXISTS shop_settings_reward_mode_check;
